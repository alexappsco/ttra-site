...
export default function Navbar({ isHome = false, isAuth = false, isLanding = false }: NavbarProps) {
  ...
  const mobileDrawer = (
    <Box sx={{ width: 250, bgcolor: isAuth ? '#000' : '#0D6EFD', color: '#fff', height: '100%' }}>
      <Box sx={{ p: 2, display: 'flex', justifyContent: 'center', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.3)' }}>
        <Image src={getLogo()} alt="Logo" width={100} height={80} />
      </Box>
      <List>
        {showNavigation &&
  ...
  return (
    <>
      <AppBar position="fixed" sx={{ top: 0, py: 2, boxShadow: 'none', bgcolor: getBackgroundColor() }}>
        <Container maxWidth="lg">
          <Card
            sx={{
              borderRadius: '16px',
              ...
              py: 0.5,
              boxShadow: '0 4px 10px rgba(0,0,0,0.1)',
              backdropFilter: showLandingLayout ? 'none' : 'blur(10px)',
              bgcolor: getCardBackground(),
              border: showLandingLayout ? '1px solid #e0e0e0' : '1px solid rgba(255,255,255,0.3)',
            }}
          >
                ...
                <>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    {showBackButton ? (
                      ...
                      <Image src={getLogo()} alt="Logo" width={110} height={80} style={{ height: 'auto', maxWidth: '100%' }} />
                    )}
                  </Box>
                  ...
                  {isDesktop && showNavigation && (
                    <Stack
                      ...
                      alignItems="center"
                      justifyContent="center"
                      sx={{ flexGrow: 1, fontSize: { xs: 13, sm: 14, md: 15 }, fontWeight: 500 }}
                    >
                      {NAV_LINKS.map((item) => (
                        <NavItem key={item.label} {...item} isHome={isHome} isAuth={isAuth} isLanding={isLanding} />
                      ))}
                    </Stack>
                  )}

                  {/* Left Section: Login / Add */}
                  {showLoginSection && !isLanding && (
                    <Stack direction="row" alignItems="center" spacing={1}>
                      <Box
                        component={NextLink}
                      ...
                      <Box
                        ...
                        sx={{
                          ...
                          textDecoration: 'none',
                          transition: 'all 0.3s ease',
                          '&:hover': { bgcolor: isHome && isAuth ? '#f5f5f5' : 'rgba(255,255,255,0.25)' },
                        }}
                      >
                        <Iconify icon="mdi:account" width={18} color={isHome && isAuth ? '#367ce5' : '#fff'} />
                        <span style={{ color: isHome && isAuth ? '#367ce5' : '#fff', fontSize: 14, fontWeight: 500 }}>
                          تسجيل دخول
                        </span>
                      </Box>
                    </Stack>
      ...
      {!isLanding && (
        <Drawer
          ...
          onClose={handleDrawerToggle}
          ModalProps={{ keepMounted: true }}
          sx={{ display: { xs: 'block', md: 'none' }, '& .MuiDrawer-paper': { width: 250, boxSizing: 'border-box' } }}
        >
          {mobileDrawer}
        </Drawer>
      )}
