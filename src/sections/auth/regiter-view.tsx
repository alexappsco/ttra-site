...
export default function JwtRegisterView({ bussiness }: Props) {
  ...
  const [currentTab, setCurrentTab] = useState(0);
  const [selectedBusiness, setSelectedBusiness] = useState<BusinessType[]>([]);
  const [serverError, setServerError] = useState<{field: 'name' | 'email' | 'root', message: string} | null>(null);
  ...
  const RegisterSchema = yup.object().shape({
    name: yup
      ...
      .matches(
        /^[a-zA-Z0-9._]{3,}$/,
        t('Global.Validation.username_invalid')
      ),
    officialName: yup.string().required(t('Global.Validation.var_required', { var: t('Global.Label.official_name') })),
    email: yup.string().email(t('Global.Validation.var_invalid', { var: t('Global.Label.email') })).required(),
    acceptTerms: yup.boolean(),
  ...
  // Handle server errors after tab change
  useEffect(() => {
    if (serverError && currentTab === 0) {
      if (serverError.field === 'name' || serverError.field === 'email') {
        setError(serverError.field, {
          type: 'server',
          message: serverError.message,
        });
      }
      // Clear the server error after setting it
      setServerError(null);
    }
  }, [currentTab, serverError, setError]);
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    if ('redirectTo' in res) {
      ...
      // Set server error state instead of directly setting form errors
      if (errorMsg.includes('username') || errorMsg.includes('user_name') || errorMsg.includes('already_exist')) {
        setServerError({
          field: 'name',
          message: t('Global.Validation.var_exists', { var: t('Global.Label.name') })
    ...
    <Box sx={{ width: '100%', height: { xs: '100vh' }, display: 'flex', flexDirection: { xs: 'column', md: 'row-reverse' } }}>
      ...
      >
        <Container maxWidth="sm" sx={{ textAlign: 'center' }}>
          ...
          <FormProvider methods={methods} onSubmit={onSubmit}>
            {/* Root error display - visible on all steps */}
            {(methods.formState.errors.root || serverError?.field === 'root') && (
              <Typography color="error" sx={{ mb: 2, textAlign: 'center' }}>
                {methods.formState.errors.root?.message || serverError?.message}
              </Typography>
            )}
